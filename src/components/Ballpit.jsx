...
function createBallpit(canvas, t = {}) {
  // FIX 1: Use window dimensions so the canvas is never set to 0×0
  // (parent offsetWidth/Height can be 0 if layout hasn't completed,
  //  which prevents the IntersectionObserver from ever firing).
  const i = new x({
    canvas,
    size: 'screen',
  ...
  // FIX 2: Apply bounds immediately — the i.resize() above ran before
  // onAfterResize was defined, so maxX/maxY were stuck at the ±5 default.
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // FIX 3: Defer via rAF so React StrictMode's synchronous unmount→remount
    // cancels the first attempt before Three.js touches the WebGL context.
    // Without this the first context gets disposed and the second mount
    // receives the same stale context, rendering nothing.
    let instance = null;
    let cancelled = false;
  ...
  // FIX 4: Canvas covers its container absolutely so there is always a
  // non-zero layout size for the IntersectionObserver to detect.
  return (
    <canvas
      ref={canvasRef}
      className={className}
